...
### R-07 年齢確認（13歳以上・未成年は保護者同意の表明）

| ID | Phase | 根拠 |
|---|---|---|
| R-07 | 1 | 規約2条 / PP 10節 / rollout「登録13歳以上・13〜17歳は保護者同意の表明・Pro 18歳以上」 |

要件
- OAuth 完了後・username 設定前の初回セットアップ画面（補遺 §2「/api/users/setup」）に年齢帯の選択を置く: 「18歳以上」「13歳以上18歳未満」「13歳未満」。生年月日は取得しない（データ最小化）。
- 「13歳以上18歳未満」を選んだ場合は「保護者の同意を得ています」チェックボックスを必須で表示する。既定は未チェック。
- 「13歳未満」を選ぶと登録を完了できず、「13歳未満の方はご利用いただけません」の案内と、作成途中の auth ユーザーの削除導線（登録取消）を出す。
...
受入基準
- 年齢帯未選択、または未成年で保護者同意チェックなしで setup API を叩くと 422 が返り、users 行が作成されない。
- 未成年ユーザーで checkout API を叩くと 403 が返る。
- 未成年ユーザーの初回公開時に注意モーダルが表示され、確認後に公開できる。
